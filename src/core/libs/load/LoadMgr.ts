class LoadMgr {
	load(url: string | (string | Laya.loadItem)[], complete?: Laya.Handler | null, progress?: Laya.Handler | null, type?: string | null, priority?: number, cache?: boolean, group?: string | null, ignoreCache?: boolean, useWorkerLoader?: boolean) {
	...
	create(url: string | (string | Laya.createItem)[], complete?: Laya.Handler | null, progress?: Laya.Handler | null, type?: string | null, constructParams?: any[] | null, propertyParams?: any, priority?: number, cache?: boolean) {
		return new Promise((resolve, reject) => {
			if ( !url || (Array.isArray(url) && url.length == 0) )
				resolve(null);
			else
	...
	loadPackage(resKey: string | string[], completeHandler?: Laya.Handler, progressHandler?: Laya.Handler) {
		return new Promise((resolve, reject) => {
			if ( !resKey || (Array.isArray(resKey) && resKey.length == 0) )
				...
				return fgui.UIPackage.loadPackage(resKey, Laya.Handler.create(this, this.checkLoaded, [ completeHandler, resolve, reject, resKey ]), progressHandler);
		});
	}
