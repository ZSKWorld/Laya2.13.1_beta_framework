...
gulp.task("preCreate_TBApp", copyLibsTask, function() {
...
gulp.task("copyPlatformFile_TBApp", versiontask, function() {
...
gulp.task("copyFiles2Pages_TBApp", ["copyPlatformFile_TBApp"], function() {
	return gulp.src([`${tempReleaseDir}/**/*.*`, `!${tempReleaseDir}/libs/**/*.*`]).pipe(gulp.dest(`${releaseDir}/pages/index`));
});

gulp.task("moveToLibs_TBApp", ["copyFiles2Pages_TBApp"], function() {
...
gulp.task("delFiles_TBApp", ["moveToLibs_TBApp"], function(cb) {
...
gulp.task("modifyFile_TBApp", ["delFiles_TBApp"], function() {
...
gulp.task("movesubpack_TBApp", ["modifyFile_TBApp"], function() {
	if (!config.taobaoInfo.subpack) { // 分包
		return;
	}
	let subpack = config.taobaoSubpack;
...
gulp.task("rmsubpack_TBApp", ["movesubpack_TBApp"], function(cb) {
...
gulp.task("modifyMinJs_TBApp", ["rmsubpack_TBApp"], function() {
	...
	if (config.keepPlatformFile) {
		...
		content = content.replace(/min\/laya(-[\w\d]+)?\.tbmini\.min\.js/gm, "laya.tbappmini.js");
	...
	let fileJsPath = path.join(releaseDir, "pages", "index", "game.js");
	let content = fs.readFileSync(fileJsPath, "utf-8");
	content = content.replace(/(min\/)?laya(-[\w\d]+)?\.tbmini(\.min)?\.js/gm, "min/laya.tbappmini.min.js");
	fs.writeFileSync(fileJsPath, content, 'utf-8');
});

gulp.task("modifyLibsJs_TBApp", ["modifyMinJs_TBApp"], function() {
...
gulp.task("version_TBApp", ["modifyLibsJs_TBApp"], function() {
	// 如果保留了平台文件，如果同时开启版本管理，就会出现文件引用不正确的问题
	if (config.keepPlatformFile) {
		let fileJsPath = path.join(releaseDir, "pages", "index", "game.js");
		let content = fs.readFileSync(fileJsPath, "utf-8");
		content = content.replace(/laya(-[\w\d]+)?\.tbmini/gm, "laya.tbappmini");
...
gulp.task("buildTBAppProj", ["version_TBApp"], function() {
