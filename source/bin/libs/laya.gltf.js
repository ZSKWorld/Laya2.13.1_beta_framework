(function (exports, Laya) {
	...
	class glTFUtils {
	    ...
	    static _getAccessorDateByteStride(componentType) {
	        switch (componentType) {
	            case 5120: return 1;
	            case 5121: return 1;
	            case 5122: return 2;
	    ...
	    static getBufferwithAccessorIndex(accessorIndex) {
	        ...
	        let componentCount = glTFUtils.getAccessorComponentsNum(accessor.type);
	    ...
	    static _initTextureData(textures, images, samplers) {
	        if (!textures || !images)
	            return;
	        textures.forEach((tex, index) => {
	            ...
	            let img = Laya.Loader.getRes(glTFImg.uri);
	            let constructParams = glTFUtils.getTextureConstructParams(glTFImg, glTFSampler);
	            let propertyParams = glTFUtils.getTexturePropertyParams(glTFSampler);
	            let tex2d = Laya.Texture2D._parse(img, propertyParams, constructParams);
	...
	class glTFLoader {
	    ...
	    static _getglTFImageUrls(glTFData, imageUrls, subUrls, urlVersion, glTFBasePath) {
	        if (glTFData.images) {
	    ...
	    static _onglTFBufferResourceLoaded(loader, processHandler, glTFData, urlVersion, glTFBasePath, processOffset, processCeil) {
	        ...
	        if (imageUrls.length > 0) {
	            ...
	            glTFLoader._innerTextureLoaderManager._create(imageUrls, false, Laya.Handler.create(null, glTFLoader._onglTFTextureResourceLoaded, [loader, process, glTFData, subUrls]), processHandler, null, null, null, 1, true);
	        }
	        else {
