window.miMiniGame = function (exports, Laya) {
    ...
    class MiniFileMgr {
        ...
        static isNetFile(url) {
            return (url.indexOf("http://") != -1 || url.indexOf("https://") != -1) && url.indexOf(KGMiniAdapter.window.qg.env.USER_DATA_PATH) == -1;
        }
        ...
        static read(filePath, encoding = "utf8", callBack = null, readyUrl = "", isSaveFile = false, fileType = "") {
            ...
            MiniFileMgr.fs.readFile({ filePath: fileUrl, encoding: encoding, success: function (data) {
                    ...
                    if (data && readyUrl != "")
                        MiniFileMgr.downFiles(KGMiniAdapter.safeEncodeURI(readyUrl), encoding, callBack, readyUrl, isSaveFile, fileType);
                    else
        ...
        static readFile(filePath, encoding = "utf8", callBack = null, readyUrl = "", isSaveFile = false, fileType = "", isAutoClear = true) {
            ...
            MiniFileMgr.fs.readFile({ filePath: filePath, encoding: encoding, success: function (data) {
                    if (filePath.indexOf(KGMiniAdapter.window.qg.env.USER_DATA_PATH) == -1 && (filePath.indexOf("http://") != -1 || filePath.indexOf("https://") != -1)) {
                        if (KGMiniAdapter.AutoCacheDownFile || isSaveFile) {
                            callBack != null && callBack.runWith([0, data]);
        ...
        static downOtherFiles(fileUrl, callBack = null, readyUrl = "", isSaveFile = false, isAutoClear = true) {
            MiniFileMgr.down({ url: fileUrl, success: function (data) {
                    if (data.statusCode === 200) {
                        if ((KGMiniAdapter.autoCacheFile || isSaveFile) && readyUrl.indexOf(".php") == -1) {
        ...
        static copyTOCache(tempFilePath, readyUrl, callBack, encoding = "", isAutoClear = true) {
            ...
            var totalSize = KGMiniAdapter.sizeLimit;
    ...
    MiniFileMgr.down = window.qg.downloadFile;
    ...
    class MiniSound extends Laya.EventDispatcher {
                ...
                if (!KGMiniAdapter.autoCacheFile) {
                    ...
                    if (MiniFileMgr.isLocalNativeFile(url)) {
                        ...
                        if (!MiniFileMgr.isLocalNativeFile(url) && (url.indexOf("http://") == -1 && url.indexOf("https://") == -1) || (url.indexOf(KGMiniAdapter.window.qg.env.USER_DATA_PATH) != -1)) {
                            ...
                            MiniFileMgr.downOtherFiles(KGMiniAdapter.safeEncodeURI(url), Laya.Handler.create(this, this.onDownLoadCallBack, [url]), url);
                        }
                    }
                }
            }
        ...
        play(startTime = 0, loops = 0) {
            ...
            if (KGMiniAdapter.autoCacheFile && MiniFileMgr.getFileInfo(this.url)) {
                ...
                tSound.src = KGMiniAdapter.safeEncodeURI(this.url);
            }
    ...
    class MiniLoader extends Laya.EventDispatcher {
            ...
            if (!KGMiniAdapter.autoCacheFile) {
                ...
                if (!MiniFileMgr.isLocalNativeFile(url) && !MiniFileMgr.getFileInfo(tempurl)) {
                    if (MiniFileMgr.isNetFile(tempurl)) {
                        MiniFileMgr.downOtherFiles(KGMiniAdapter.safeEncodeURI(tempurl), Laya.Handler.create(MiniLoader, MiniLoader.onDownLoadCallBack, [tempurl, thisLoader]), tempurl);
                    }
                    else {
                        MiniLoader.onDownLoadCallBack(url, thisLoader, 0);
                    }
        ...
        static onDownLoadCallBack(sourceUrl, thisLoader, errorCode, tempFilePath = null) {
            if (!errorCode) {
                ...
                sound.load(KGMiniAdapter.safeEncodeURI(sourceUrl));
        ...
        _loadHttpRequestWhat(url, contentType) {
            ...
            if (Laya.Loader.preLoadedMap[tempurl])
                ...
                if (!KGMiniAdapter.AutoCacheDownFile) {
                    if (MiniFileMgr.isNetFile(tempurl)) {
                    ...
                    if (!MiniFileMgr.isLocalNativeFile(url) && !MiniFileMgr.getFileInfo(tempurl)) {
                        if (MiniFileMgr.isNetFile(tempurl)) {
                            MiniFileMgr.downFiles(KGMiniAdapter.safeEncodeURI(tempurl), encoding, new Laya.Handler(MiniLoader, MiniLoader.onReadNativeCallBack, [url, contentType, thisLoader]), tempurl, true);
                        }
                        else {
                            MiniFileMgr.readFile(url, encoding, new Laya.Handler(MiniLoader, MiniLoader.onReadNativeCallBack, [url, contentType, thisLoader]), url);
                        }
        ...
        static onReadNativeCallBack(url, type = null, thisLoader = null, errorCode = 0, data = null) {
            if (!errorCode) {
                ...
                if (!KGMiniAdapter.isZiYu && KGMiniAdapter.isPosMsgYu && type != Laya.Loader.BUFFER) {
                    KGMiniAdapter.window.qg.postMessage({ url: url, data: tempData, isLoad: "filedata" });
                }
        ...
        static _transformImgUrl(url, type, thisLoader) {
            ...
            if (!KGMiniAdapter.autoCacheFile) {
                thisLoader._loadImage(KGMiniAdapter.safeEncodeURI(url));
            }
            else {
                var tempUrl = Laya.URL.formatURL(url);
                if (!MiniFileMgr.isLocalNativeFile(url) && !MiniFileMgr.getFileInfo(tempUrl)) {
                    if (MiniFileMgr.isNetFile(tempUrl)) {
                        MiniFileMgr.downOtherFiles(KGMiniAdapter.safeEncodeURI(tempUrl), new Laya.Handler(MiniLoader, MiniLoader.onDownImgCallBack, [url, thisLoader]), tempUrl);
                    }
                    else {
    ...
    class KGMiniAdapter {
        ...
        static init(isPosMsg = false, isSon = false) {
            ...
            if (!KGMiniAdapter.isZiYu) {
                ...
                MiniFileMgr.getCacheList(MiniFileMgr.fileNativeDir, Laya.Handler.create(KGMiniAdapter, KGMiniAdapter.onMkdirCallBack));
            }
            ...
            Laya.HttpRequest._urlEncode = KGMiniAdapter.safeEncodeURI;
        ...
        static onMkdirCallBack(errorCode, data) {
            ...
            let files;
            try {
                files = MiniFileMgr.fs.readdirSync(MiniFileMgr.fileNativeDir);
            }
            catch (error) {
                console.log("readdirsync failed. path: " + MiniFileMgr.fileNativeDir + " error: " + error);
            }
    ...
    KGMiniAdapter.IGNORE = new RegExp("[-_.!~*'();/?:@&=+$,#%]|[0-9|A-Z|a-z]");
    KGMiniAdapter.safeEncodeURI = function (str) {
        var strTemp = "";
        var length = str.length;
        for (var i = 0; i < length; i++) {
            var word = str[i];
            if (KGMiniAdapter.IGNORE.test(word)) {
                strTemp += word;
            }
            else {
                try {
    ...
    KGMiniAdapter.sizeLimit = (50 * 1024 * 1024);
