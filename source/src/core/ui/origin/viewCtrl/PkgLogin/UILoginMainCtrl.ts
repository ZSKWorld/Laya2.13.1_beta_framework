...
export class UILoginMainCtrl extends BaseViewCtrl<UILoginMainView, UILoginMainData>{
    ...
    private onBtnRegisterClick(): void {
        ...
        if (!TxtRegisterAccount.text.trim()) tipMgr.showTip("请输入账号");
        else if (!TxtRegisterPassword.text.trim()) tipMgr.showTip("请输入密码");
        else if (!TxtRegisterName.text.trim()) tipMgr.showTip("请输入昵称");
        else {
