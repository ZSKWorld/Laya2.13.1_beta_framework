...
export class RenderChooseBattleView extends ExtensionClass<IView, RenderChooseBattle>(RenderChooseBattle) {
    ...
    refreshFuBen(data: ConfigFuBenData) {
        ...
        this.TxtContent3.text = `剩余次数：${ userData.getCopyTime(data.ID) }`;
    }

    refreshMiJing(data: ConfigMiJingData) {
        this.ctrlState.selectedIndex = 2;
        this.TxtContent1.text = "Lv." + data.EnemyLevel + " " + data.Name;
        this.TxtContent3.text = `剩余次数：${ userData.getSecretTime(data.ID) }`;
    }
    ...
    private refreshBossCool() {
        ...
        const coolTime = userData.getBossCoolDown(this._bossData.ID);
