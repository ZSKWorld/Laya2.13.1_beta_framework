...
export class KeyFrame {
    ...
    static getData(x: number, y: number, inTangent: number, outTangent: number) {
        const data = Pool.getItemByClass("DuelKeyFrame", KeyFrame) as KeyFrame;
        data.x = x;
        data.y = y;
    ...
    static recoverData(data: KeyFrame) {
        ...
        Pool.recover("DuelKeyFrame", data);
    }
}
...
export class CurveData {
    ...
    static getData() {
        return Pool.getItemByClass("DuelAnimationCurve", CurveData) as CurveData;
    }

    static recoverData(data: CurveData) {
        data.propertyeType = AnimationPropertyType.None;
        data.curves.forEach(v => v.forEach(KeyFrame.recoverData));
        data.curves.length = 0;
        Pool.recover("DuelAnimationCurve", data);
    }
}

export class AnimationCurve extends Script3D {
    override readonly owner: Sprite3D;
    private _datas: CurveData[] = [];
    ...
    private reset() {
        this._datas.forEach(CurveData.recoverData);
